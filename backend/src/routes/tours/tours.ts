...
toursRouter.post('/', auth, permit('ADMIN', 'MANAGER'), imagesUpload.array('images', 5), async (req, res, next) => {
    try {
        const {title, description, category, baseAdvantages} = req.body;

        const parsedAdvantages: string[] = typeof baseAdvantages === 'string'
            ? baseAdvantages.split(',').map(a => a.trim()).filter(Boolean)
            : (Array.isArray(baseAdvantages) ? baseAdvantages : []);
...
toursRouter.patch('/:id', auth, permit('ADMIN', 'MANAGER'), imagesUpload.array('images', 5), async (req, res, next) => {
    const {id} = req.params;
    ...
    try {
        ...
        const {title, description, category, baseAdvantages, isPublished} = req.body;
        ...
        if (baseAdvantages) {
            tour.baseAdvantages = typeof baseAdvantages === 'string'
                ? baseAdvantages.split(',').map(a => a.trim()).filter(Boolean)
                : (Array.isArray(baseAdvantages) ? baseAdvantages : tour.baseAdvantages);
        }

        if (req.files && (req.files as Express.Multer.File[]).length > 0) {
            const newImages = (req.files as Express.Multer.File[]).map(file => 'images/' + file.filename);
