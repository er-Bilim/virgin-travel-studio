...
const PublicTourCard = ({tour, tourSets}: Props) => {
    const image = tour.images.length > 0 ? imageUrl + tour.images[0] : BasePhoto.src;

    const availableTourSets = tourSets.filter(
        (tourSet) => tourSet.tourId._id === tour._id && tourSet.status !== 'FINISHED',
    );
                                        ...
                                        <div className="flex items-start justify-between gap-3">
                                            <div>
                                                <div className="mb-2 flex flex-wrap items-center gap-2">
                                                    {tourSet.isHot && (
                                                        <span
                                                            className="rounded-full bg-red-500 px-3 py-1 text-xs font-bold uppercase text-white">
                              HOT
                            </span>
                                                    )}

                                                    <span className="text-xs text-white/70">
                            {new Date(tourSet.startDate).toLocaleDateString('ru-RU')} —{' '}
                                                        {new Date(tourSet.endDate).toLocaleDateString('ru-RU')}
                          </span>
                                                </div>
