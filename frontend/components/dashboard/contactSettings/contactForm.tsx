...
import { mutateContacts, mutateCreateContacts, useContacts } from '@/lib/hooks/contactSettings';
...
export default function ContactSettingsForm() {
  ...
  const onSubmit = (data: IContactSettings) => {
    ...
    if (formattedData.whatsapp) formData.append('whatsapp', formattedData.whatsapp);
    if (formattedData.telegram) formData.append('telegram', formattedData.telegram);
    if (formattedData.instagram) formData.append('instagram', formattedData.instagram);
    if (formattedData.facebook) formData.append('facebook', formattedData.facebook);
    if (formattedData.mapEmbedUrl) formData.append('mapEmbedUrl', formattedData.mapEmbedUrl);

    if (formattedData.workingHours) {
      formData.append('workingHours', JSON.stringify(formattedData.workingHours));
    }
  ...
  return (
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 flex-1 gap-4">
          ...
          <Field label="Логотип:" error={errors.logo?.message as string | undefined}>
            <Input
                type="file"
                accept="image/*"
                {...register('logo')}
          ...
          {typeof contactSettings?.logo === 'string' && contactSettings.logo && (
              <div className="space-y-1">
                <p className="text-sm font-medium text-gray-700">Текущий логотип:</p>
                <img
                    src={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/'}${contactSettings.logo}`}
                    alt="Текущий логотип"
                    className="h-12 w-auto rounded border bg-gray-50 p-1"
                />
              </div>
          )}
        </div>
          ...
          <div className="rounded-2xl border border-gray-100 p-4 space-y-4 divide-y">
            <div className="pb-4">
              ...
              <div className="flex items-center gap-3">
                <Field label="С" error={errors.workingHours?.weekdays?.from?.message}>
                  <Input
                ...
                <Field label="До" error={errors.workingHours?.weekdays?.to?.message}>
                  <Input
