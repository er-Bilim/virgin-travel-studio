...
export default function OrderManageForm({ initialValues, orderId }: Props) {
  ...
  const statusText = ORDER_STATUS_FLOW.map((status) => ({
    status,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 w-full">
      <div className="space-y-4">
                ...
                <div className="w-full">
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <p className="text-[12px] text-slate-400 max-w-md leading-relaxed">
                      Если заявку нельзя выполнить по какой-либо причине —
                      отклоните её с указанием обязательного комментария для
                      истории.
                    </p>
        ...
        <div className="space-y-1.5 pt-1">
          ...
          <Input
            ...
            className={cn(
              inputClass,
              'font-semibold bg-slate-50 border-slate-100 rounded-xl h-11 text-sm text-red-600/90',
            )}
          />
        </div>
      </div>
      ...
      <div className="pt-2 flex items-center justify-end">
        <Button
          type="submit"
          className="w-full sm:w-auto h-11 px-6 rounded-xl bg-[#031633] text-white font-semibold hover:bg-[#031633]/90 transition shadow-sm cursor-pointer text-sm"
        >
          Сохранить изменения
        </Button>
      </div>
